...
<div class="relative mx-auto h-[84px] w-[112px]">
	<div class="relative flex h-full w-full flex-col items-center">
		...
		<div
			class="relative flex h-[63px] w-[98px] items-center justify-center overflow-visible rounded-lg border-2 border-sky-500 bg-sky-600 transition-transform duration-200 ease-in-out"
			style:transform="scaleX({scale})"
		>
			<!-- Left Hand Pushing Right -->
			<div
				class="absolute top-1/2 -left-8 -mt-4 -translate-y-1/2"
			...
			<div
				class="absolute top-1/2 -right-8 -mt-4 -translate-y-1/2"
				style:transform="scaleX({1 / scale})"
			>
			...
			<div
				class="z-2 text-base font-semibold whitespace-nowrap text-white drop-shadow-[1px_1px_0px_rgba(0,0,0,0.2)]"
				style:transform="scaleX({1 / scale})"
			>
				{value}
				{unit}
			...
			></div>
			<div
				class="absolute top-1/2 right-[10px] left-[10px] h-px bg-black/10 before:absolute before:inset-x-0 before:-top-[20px] before:h-px before:bg-white/10 after:absolute after:inset-x-0 after:top-[20px] after:h-px after:bg-black/10"
			></div>
		</div>
	</div>
</div>
